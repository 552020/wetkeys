...
export interface Agent {
  call: (canisterId: string, method: string, args: any[]) => Promise<any>;
}
...
export class VetKeysManager {
   ...
   */
  async uploadFile(name: string, content: Uint8Array, fileType: string, sharedWith: Principal[] = []): Promise<number> {
    try {
      const request = {
        name,
   ...
   * @returns {Promise<FileData>} File data
   */
  async downloadFile(fileId: number, chunkId: number = 0): Promise<FileData> {
    try {
   ...
   */
  async shareFile(fileId: number, sharedWith: Principal[]): Promise<boolean> {
    try {
      const request = {
        file_id: fileId,
        shared_with: sharedWith.map(p => p.toText()),
   ...
   */
  static fromText(principalText: string): Principal {
 ...
 * @param {Agent} agent - Internet Computer agent
 * @returns {VetKeysManager} VetKeys manager instance
 */
export function createVetKeysManager(agent: Agent): VetKeysManager {
