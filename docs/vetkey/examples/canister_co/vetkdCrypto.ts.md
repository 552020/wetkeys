...
export class VetkdCryptoService {
  ...
  async encrypt(data: ArrayBuffer, userPrincipalBytes: Uint8Array): Promise<Uint8Array> {
    try {
      ...
      const encryptedData = vetkd.IBECiphertext.encrypt(publicKey, userPrincipalBytes, encodedMessage, seed);
    ...
    try {
      ...
      if (isSharedFile) {
        ...
        try {
          ...
          console.log("Using owner's principal for shared file:", principalToUse);
        } catch (e) {
          console.error("Failed to get owner principal, falling back to user principal", e);
        }
      } else {
      ...
      if (!privateKeyResponse || "Err" in privateKeyResponse) {
        throw new Error(
          "Error getting encrypted key: " + ("Err" in privateKeyResponse ? privateKeyResponse.Err : "empty response")
        );
      }
